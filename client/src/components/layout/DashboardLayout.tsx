...
export function DashboardLayout() {
  ...
  const roleItems = navRole !== undefined ? SIDEBAR_ITEMS[navRole] ?? [] : [];
  const adminItems: AppSidebarItem[] =
    user?.platformRole === "SUPER_ADMIN"
      ? [{ to: ROUTES.ADMIN.PROCESSES, label: "Process FTI", icon: Workflow }]
      : [];
  // Keep legacy role landing first; platform administration is an additional axis.
  const sidebarItems = [...roleItems, ...adminItems];
