...
print_migration_output() {
  # Redact URL passwords defensively in case a future Prisma version logs one.
...
run_migrations() {
  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    ...
    # Retry only transient reachability failures. Migration errors such as
    # P3009/P3018 must stop immediately for explicit recovery.
