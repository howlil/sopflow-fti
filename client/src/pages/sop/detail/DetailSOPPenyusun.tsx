...
import { BuatVersiBaruDialog } from '@/pages/sop/components/BuatVersiBaruDialog'
import {
...
import type { SopProsedurAutosaveStatus } from '@/pages/sop/hooks/use-sop-prosedur-autosave'
...
export function DetailSOPPenyusun() {
  const { id } = useParams({ from: '/sop/$id' })
  ...
  const canBuatVersiBaru = canManageSop && buatVersiBaruBlockingReason === null
  ...
  useEffect(() => {
    ...
    if (autosaveError && autosaveError !== lastHeaderErrorRef.current) {
      ...
      showToast(`Gagal menyimpan otomatis header SOP: ${autosaveError.message}`, 'error')
    }
  ...
  useEffect(() => {
    ...
    if (prosedurAutosaveError && prosedurAutosaveError !== lastProsedurErrorRef.current) {
      ...
      showToast(
        `Gagal menyimpan otomatis langkah/aktor pelaksana: ${prosedurAutosaveError.message}`,
        'error',
      )
    }
  ...
  return (
    <SopEditorProvider value={editorContextValue}>
      <DetailPageLayout
        breadcrumb={[
          { label: 'SOP', to: ROUTES.SOP },
          { label: isReadOnly ? 'Lihat SOP' : 'Ubah SOP' },
        ]}
        title={isReadOnly ? 'Lihat Dokumen SOP' : 'Ubah Dokumen SOP'}
        ...
        header={
          <DetailSOPPenyusunHeader
            ...
            onBuatVersiBaru={() => {
              if (currentVersionSource && canManageSop) setBuatVersiSource(currentVersionSource)
            }}
        ...
        rightPanel={
          <DetailSOPPenyusunSidePanel
            ...
            editTabLabel={isReadOnly ? 'Informasi' : 'Ubah'}
            isReadOnly={isReadOnly}
            canReview={canReviewProsesBisnis}
            detailSopId={id}
            sopId={sopHeaderId}
            onBuatVersiBaru={canManageSop ? setBuatVersiSource : undefined}
            isBuatVersiBaruPending={isBuatVersiBaruPending}
