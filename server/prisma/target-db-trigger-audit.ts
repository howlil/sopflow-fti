...
const expectedTriggerNames = new Set([
  ...
  'trg_process_owner_authority_one_scope_insert',
...
const semanticRequirements: Record<string, string[]> = {
  ...
  trg_process_owner_authority_one_scope_insert: ['`penggunaId`', '`revokedAt`'],
  trg_process_owner_scope_insert: ['`ownerId`', '`scope`', '`departmentId`', '`ProcessOwnerAuthority`'],
};
