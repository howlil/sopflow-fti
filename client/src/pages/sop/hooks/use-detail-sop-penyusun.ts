...
import { buildSopHeaderSnapshot, useSopHeaderAutosave, type SopHeaderAutosaveStatus } from "@/pages/sop/hooks/use-sop-header-autosave";
import { buildSopProsedurSnapshot, useSopProsedurAutosave, type SopProsedurAutosaveStatus } from "@/pages/sop/hooks/use-sop-prosedur-autosave";
...
export interface UseDetailSopPenyusunDataResult {
  ...
  setRightPanelTab: React.Dispatch<
    React.SetStateAction<"edit" | "versi" | "review">
  >;
  isLoading: boolean;
  ...
  /** True jika akun adalah Anggota Proses Bisnis dan boleh melakukan aksi authoring/versioning. */
  canManageSop: boolean;
  /** True jika akun adalah Penanggung Jawab Proses Bisnis untuk review SOP aktif. */
  canReviewProsesBisnis: boolean;
}
...
export function useDetailSopPenyusunData(
  ...
  const canManageSop = workbench?.canEdit ?? true;
  const canReviewProsesBisnis =
    workbench?.siklus?.stage === "PROCESS_REVIEW" &&
    workbench.siklus.responsibility.type === "CURRENT_USER";
  const canEditDetail = canManageSop && canEditSop(resolvedStatusForEdit);
  useEffect(() => {
    if (canReviewProsesBisnis) setRightPanelTab("review");
  }, [canReviewProsesBisnis, sopDetailId]);
...
export function useDetailSopPenyusun(
  ...
  return {
    ...
    canManageSop: data.canManageSop,
    canReviewProsesBisnis: data.canReviewProsesBisnis,
  };
}
