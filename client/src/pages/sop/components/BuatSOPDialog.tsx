...
export interface BuatSOPDialogProps {
  ...
  prosesBisnis: ProsesBisnisDto[];
  isLoadingProsesBisnis: boolean;
      ...
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            SOP diinisiasi oleh Penanggung Jawab Proses Bisnis.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3 pt-1">
          <FormField label="Proses Bisnis" required>
            <select
              ...
              disabled={isLoadingProsesBisnis}
              onChange={(event) =>
                setFormData((prev) => ({ ...prev, prosesBisnisId: event.target.value }))
              }
            >
              <option value="">
                {isLoadingProsesBisnis ? "Memuat Proses Bisnis..." : "Pilih Proses Bisnis"}
              </option>
            ...
            {!isLoadingProsesBisnis && prosesBisnis.length === 0 ? (
              <p className="mt-1 text-xs text-secondary-foreground">
                Anda belum memiliki Proses Bisnis sebagai Penanggung Jawab.
              </p>
            ) : null}
          </FormField>
