...
vi.mock("@/components/layout/SidebarUserMenu", () => ({
  SidebarUserMenu: ({ collapsed = false }: { collapsed?: boolean }) => (
    <button type="button" aria-label={collapsed ? "Menu profil uji collapsed" : "Menu profil uji"}>
      Profil
    </button>
  ),
}));
...
describe("DashboardLayout desktop sidebar", () => {
  ...
  it("mengisolasi workflow role lama ketika akun memiliki Process", () => {
    ...
    expect(screen.queryByRole("link", { name: "SOP" })).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "Pelaksana SOP" })).not.toBeInTheDocument();
    expect(screen.queryByRole("link", { name: "Peraturan" })).not.toBeInTheDocument();
  });
  ...
  it("tidak menyediakan menu role lama saat akun belum memiliki konteks target", () => {
    render(<DashboardLayout />);
