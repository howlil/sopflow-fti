...
schema = schema.replace(
    '@@index([prosesBisnisId], map: "DokumenTte_prosesBisnisId_idx")',
    '@@index([prosesBisnisId], map: "DokumenTte_processId_idx")',
)
start = schema.index('model UndanganAnggotaProsesBisnis {')
end = schema.index('\n}', start) + 2
block = schema[start:end]
for old, new in {
    'email               String                  @db.VarChar(255)': 'email               String                  @db.VarChar(31)',
    'nama                String                  @db.VarChar(255)': 'nama                String                  @db.VarChar(31)',
    'nip                 String                  @db.VarChar(32)': 'nip                 String                  @db.Char(18)',
    'jabatan             String                  @db.VarChar(255)': 'jabatan             String                  @db.VarChar(28)',
    'pangkat             String                  @db.VarChar(64)': 'pangkat             String                  @db.VarChar(25)',
    'nohp                String                  @db.VarChar(32)': 'nohp                String                  @db.VarChar(13)',
}.items():
    if old not in block:
        raise SystemExit(f'Expected ProcessInvitation schema line not found: {old}')
...
table_start = baseline.index('CREATE TABLE `ProcessInvitation` (')
table_end = baseline.index(') DEFAULT CHARACTER SET', table_start)
invitation = baseline[table_start:table_end]
for old, new in {
    '`email` VARCHAR(255) NOT NULL': '`email` VARCHAR(31) NOT NULL',
    '`nama` VARCHAR(255) NOT NULL': '`nama` VARCHAR(31) NOT NULL',
    '`nip` VARCHAR(32) NOT NULL': '`nip` CHAR(18) NOT NULL',
    '`jabatan` VARCHAR(255) NOT NULL': '`jabatan` VARCHAR(28) NOT NULL',
    '`pangkat` VARCHAR(64) NOT NULL': '`pangkat` VARCHAR(25) NOT NULL',
    '`nohp` VARCHAR(32) NOT NULL': '`nohp` VARCHAR(13) NOT NULL',
}.items():
    if old not in invitation:
        raise SystemExit(f'Expected ProcessInvitation baseline line not found: {old}')
    invitation = invitation.replace(old, new, 1)
baseline = baseline[:table_start] + invitation + baseline[table_end:]
...
'''
if 'OrganizationalAuthorityAssignment_departmentId_fkey' not in baseline:
    baseline = baseline.rstrip() + missing_fks + '\n'
baseline_path.write_text(baseline)

invariants_path = Path('server/prisma/migrations/1_fti_native_invariants/migration.sql')
invariants = invariants_path.read_text()
start_marker = '-- Normalize DetailSOP trigger state without depending on superseded trigger names.'
create_marker = 'CREATE TRIGGER `trg_detailsop_one_effective_insert`'
start = invariants.index(start_marker)
create = invariants.index(create_marker)
replacement = '''-- Replace the one-active-version invariant deterministically.\nDROP TRIGGER IF EXISTS `trg_detailsop_one_berlaku_insert`;\nDROP TRIGGER IF EXISTS `trg_detailsop_one_berlaku_update`;\nDROP TRIGGER IF EXISTS `trg_detailsop_one_effective_insert`;\nDROP TRIGGER IF EXISTS `trg_detailsop_one_effective_update`;\n\n'''
invariants = invariants[:start] + replacement + invariants[create:]
invariants_path.write_text(invariants)
