...
vi.mock('@/pages/sop/detail/SopEditorContext', () => ({
  useSopEditor: () => ({
...
vi.mock('@/pages/sop/detail/components/DetailSopProsedurEditor', () => ({
  DetailSOPProsedurEditor: () => <div>Procedure editor</div>,
}))

vi.mock('@/pages/sop/hooks/use-penyusun-diagram-config', () => ({
...
import { DetailSOPPenyusunMain } from '@/pages/sop/detail/components/DetailSopPenyusunMain'
...
describe('DetailSOPPenyusunMain toolbar', () => {
  ...
  it('menandai mode langkah dan edit path dengan aria-pressed', () => {
    ...
    expect(screen.getByRole('button', { name: 'Ubah Manual' })).toHaveAttribute(
      'aria-pressed',
      'false',
    )
  })
  ...
  it('menyembunyikan kontrol edit saat mode read-only', () => {
    ...
    expect(screen.queryByRole('button', { name: 'Ubah Manual' })).not.toBeInTheDocument()
  })
})
