...
/**
 * Mengumpulkan pesan validasi kelengkapan workbench sebelum masuk Pemeriksaan Proses Bisnis.
...
export function pastikanWorkbenchSopLengkapUntukPemeriksaanProsesBisnis(row: SopWorkbenchDbPayload): void {
  ...
  if (pesan.length > 0) {
    throw new BadRequestException(`SOP belum lengkap untuk Pemeriksaan Proses Bisnis. ${pesan.join(' ')}`);
  }
}
